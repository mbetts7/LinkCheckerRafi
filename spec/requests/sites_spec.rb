...
describe "Sites" do
	...
	describe "GET /sites/:id/edit" do
		before do
			@site = Site.create!(:url => "links-here.com")
		end
	...
	describe "GET /linkfarm/" do
		before do
			@site1 = Site.create!(:url => "no-links-here.com")
			@site2 = Site.create!(:url => "links-here.com")
			
			@site1.links.create(:url => "espn.com")
			@site1.links.create(:url => "nba.com")

			@site2.links.create(:url => "fly.com")
			@site2.links.create(:url => "bye.com")
		end
		...
		it 'gets the links with HTML' do
			...
			response.should be_success
			response.body.should include(@site1.url)
		end
		...
		it 'gets the links with JSON' do
			...
			r.first['url'].should == @site1.url
			JSON.parse(response.body).first["url"].should == @site.url
